...
def grouped_places_boxplot_devices(data_dict, train_device=None,
    ...
    plt.legend(handles=patches, loc=2, framealpha=0.5)

    plt.ylabel("error(m)")

    plt.title("Device Heterogenity (STI-WELM) " + str(train_device))

    # plt.savefig(outfile)
    # plt.savefig("test"+train_device+".PNG")
